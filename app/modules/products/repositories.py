...
class ProductRepository:
    ...
    async def create_product(self,seller_id: int, product: dict):
        product_db = Product(seller_id=seller_id, **product)
    ...
    async def delete_product(self, product_id: int, user_id: int):
        await self.db.execute(update(Product).where(Product.id == product_id, Product.seller_id==user_id).values(is_active=False))
        await self.db.commit()
