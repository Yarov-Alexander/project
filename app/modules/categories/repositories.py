...
class CategoryRepository:
    ...
    async def update_category(self, category_id: int, category_data: dict) -> Category:
        await self.db.execute(update(Category).where(Category.id == category_id).values(**category_data))
        await self.db.commit()
